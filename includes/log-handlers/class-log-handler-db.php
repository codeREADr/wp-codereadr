...
namespace CodeReadr\Log_Handlers;

use CodeReadr\Abstracts\Log_Handler;
...
class Log_Handler_DB extends Log_Handler {
	...
	public function handle( $timestamp, $level, $message, $context ) {
		...
		$context['versions']['CodeReadr'] = CODEREADR_VERSION;
	...
	public static function get_all( $levels = false, $offset = 0, $count = 10000000 ) {
		...
		$results = $wpdb->get_results(
			...
			$wpdb->prepare(
				"
					SELECT *
					FROM {$wpdb->prefix}codereadr_log
					$where
		...
		foreach ( $results as $result ) {
			...
			$plugin         = esc_html__( 'Core', 'codereadr' );
	...
	public static function get_count( $levels = false ) {
		...
		return (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}codereadr_log $where" ); // phpcs:ignore
	}
	...
	public static function flush() {
		...
		return $wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}codereadr_log" );
	}
	...
	public function clear( $source ) {
		...
		return $wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}codereadr_log WHERE source = %s",
				$source
			)
		);
	}
	...
	public static function delete( $log_ids ) {
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}codereadr_log WHERE log_id IN {$query_in}", $log_ids ) ); // @codingStandardsIgnoreLine.
	}
	...
	public static function delete_logs_before_timestamp( $timestamp = 0 ) {
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$wpdb->prefix}codereadr_log WHERE timestamp < %s",
	...
	protected static function get_log_source() {
		static $ignore_classes = array( 'CodeReadr\Log_Handlers\Log_Handler_DB', 'CodeReadr\Logger' );
